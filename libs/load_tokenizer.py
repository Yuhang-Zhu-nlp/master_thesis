...
def load_tokenizer_model(model_name: str, pool_method: str):
    if model_name == 'en_bert':
        tokenizer = BertTokenizer.from_pretrained('bert-large-cased')
        model = english_bert(pool_method=pool_method)
    elif model_name == 'xlm-roberta':
        tokenizer = AutoTokenizer.from_pretrained('xlm-roberta-large')
        model = xlm_roberta_pooling_representation(pool_method=pool_method)
    elif model_name == 'ernie':
        tokenizer = AutoTokenizer.from_pretrained('susnato/ernie-m-large_pytorch')
        model = erniem_pooling_representation(pool_method=pool_method)
    else:
